...
impl OnceCell {
    ...
    pub fn init(&self, val: u32) -> bool {
        ...
        match self.initialized.compare_exchange(false, true, Ordering::AcqRel, Ordering::Acquire) {
            Ok(_)=>{
    ...
    pub fn get(&self) -> Option<u32> {
        ...
        // todo!()
        let status = self.initialized.load(Ordering::Acquire);
        if !status {
            return None;
        }else {
