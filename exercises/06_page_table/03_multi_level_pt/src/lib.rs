...
impl Sv39PageTable {
    ...
    /// 获取页表节点的可变引用
    fn get_node_mut(&mut self, ppn: u64) -> Option<&mut PageTableNode> {
        self.nodes.get_mut(&ppn)
    }
    ...
    /// 获取页表节点的不可变引用
    fn get_node(&self, ppn: u64) -> Option<&PageTableNode> {
        self.nodes.get(&ppn)
    }
    ...
    pub fn map_page(&mut self, va: u64, pa: u64, flags: u64) {
        ...
        for level in (1..=2).rev() {
            let vpn_idx = Sv39PageTable::extract_vpn(va, level);
            
            // 检查是否需要创建新节点
            let need_new_node = {
                let node = self.get_node(current_ppn).unwrap();
                node.entries[vpn_idx] & PTE_V == 0
            };
            
            if need_new_node {
                let new_ppn = self.alloc_node();
                let node = self.get_node_mut(current_ppn).unwrap();
                node.entries[vpn_idx] = (new_ppn << PPN_SHIFT) | PTE_V;
            }
            
            // 进入下一级页表
            let node = self.get_node(current_ppn).unwrap();
            current_ppn = (node.entries[vpn_idx] >> PPN_SHIFT) & ((1 << 44) - 1);
        }
        
        // 处理最后一级（level 0）
        let vpn_idx = Sv39PageTable::extract_vpn(va, 0);
        let node = self.get_node_mut(current_ppn).unwrap();
        node.entries[vpn_idx] = (pa >> 12) << PPN_SHIFT | flags;
    }
    ...
    pub fn translate(&self, va: u64) -> TranslateResult {
        ...
        for level in (0..=2).rev() {
            let vpn_idx = Sv39PageTable::extract_vpn(va, level);
            let node = match self.get_node(current_ppn) {
                Some(n) => n,
                None => return TranslateResult::PageFault,
            };
            let pte = node.entries[vpn_idx];
            ...
            // 检查是否是叶节点 (R、W、X 有任一置位)
            if pte & (PTE_R | PTE_W | PTE_X) != 0 {
                // 叶节点，提取 PPN 并计算物理地址
                let ppn = (pte >> PPN_SHIFT) & ((1 << 44) - 1);
                let offset = va & ((1 << (12 + level * 9)) - 1);
                return TranslateResult::Ok((ppn << 12) | offset);
            }
            
            // 非叶节点，进入下一级页表
            current_ppn = (pte >> PPN_SHIFT) & ((1 << 44) - 1);
        }
        
        // 理论上不会到达这里，因为 level 0 必须是叶节点
    ...
    pub fn map_superpage(&mut self, va: u64, pa: u64, flags: u64) {
        ...
        // 只需要遍历到 level 2，创建中间页表节点
        let vpn_idx = Sv39PageTable::extract_vpn(va, 2);
        ...
        // 在 level 1 写入叶子 PTE（大页映射）
        let vpn_idx = Sv39PageTable::extract_vpn(va, 1);
        let node = self.get_node_mut(current_ppn).unwrap();
        node.entries[vpn_idx] = (pa >> 12) << PPN_SHIFT | flags;
    }
}
